...
import { TeachingLocation, type Course } from '../../types';
...
const EditCourseForm = ({ course, onSuccess, onCancel }: EditCourseFormProps) => {
  ...
  useEffect(() => {
    if (course) {
      setFormData({
        ...
        hourlyRate: course.hourlyRate ? course.hourlyRate.toString() : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Une erreur est survenue lors de la modification du cours');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-xl font-bold text-gray-900 mb-6">Modifier le cours</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center">
              <input
                ...
                checked={formData.locations.includes(TeachingLocation.ONLINE)}
            ...
            <div className="flex items-center">
              <input
                ...
                checked={formData.locations.includes(TeachingLocation.HOME)}
                onChange={handleLocationChange}
                className="mr-2"
              />
            ...
            <div className="flex items-center">
              <input
                ...
                checked={formData.locations.includes(TeachingLocation.TEACHER_PLACE)}
                onChange={handleLocationChange}
                className="mr-2"
              />
        ...
        <div className="flex justify-end space-x-3 pt-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Modification en cours...' : 'Modifier le cours'}
          </button>
        </div>
      </form>
    </div>
